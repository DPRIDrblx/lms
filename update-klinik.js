...
import { BookOpen, Calendar, Clock, CheckCircle2, ChevronRight, Star, AlertCircle, Plus, Users, Beaker } from "lucide-react";
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {loading ? (
                  ...
                  filteredClinics.map(clinic => {
                    ...
                    return (
                      <div key={clinic.id} className="bg-white rounded-[20px] border border-slate-200 overflow-hidden flex flex-col shadow-sm hover:shadow-md hover:border-slate-300 transition-all">
                        <div className="h-28 bg-[#E6F6F4] relative overflow-hidden flex items-center justify-center">
                           ...
                           <div className="absolute top-3 left-3">
                             <span className={cn("px-2.5 py-1 rounded-[8px] text-[10px] font-black uppercase tracking-wider", 
                               clinic.status === 'completed' ? "bg-emerald-100 text-emerald-700" :
                               clinic.status === 'approved' ? "bg-blue-100 text-blue-700" :
                           ...
                           {clinic.status === 'completed' && !clinic.rating && (
                             <div className="absolute top-3 right-3">
                               <span className="px-2.5 py-1 rounded-[8px] text-[10px] font-black uppercase tracking-wider bg-orange-100 text-orange-700 flex items-center gap-1">
                                 <AlertCircle className="w-3 h-3" /> Belum Rating
                               </span>
                             </div>
                           )}
                        </div>
                        <div className="p-5 flex-1 flex flex-col">
                          <h3 className="text-lg font-black text-slate-800 mb-1 leading-tight">{clinic.subject}</h3>
                          ...
                          <div className="grid grid-cols-2 gap-2 mt-auto">
                            ...
                            {clinic.status === 'completed' && !clinic.rating && (
                              <Button 
                                ...
                                className="w-full h-10 bg-orange-500 hover:bg-orange-600 text-white rounded-[10px] text-[13px] font-bold"
                              >
                                Beri Rating
                              </Button>
                            )}
