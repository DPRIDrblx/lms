      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <Badge variant={course?.is_published ? "success" : "info"} className="mt-1">
              {course?.is_published ? "Published" : "Draft"}
            </Badge>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="secondary" onClick={() => setShowChapterModal(true)} icon={<Plus className="h-4 w-4" />}>
            New Chapter
          </Button>
          <Button onClick={() => {
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="p-5 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Link href={`/teacher/grading/offline?course_id=${id}`}>
                <Button variant="secondary" className="w-full justify-start" icon={<Award className="h-4 w-4" />}>
                  Offline Grading
                </Button>
              </Link>
            </div>
                  ...
                  <div key={chapter.id} className="space-y-2">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <Badge variant="info" className="text-[10px]">{chapterLessons.length + chapterQuizzes.length} Items</Badge>
                      </div>
                      <button 
                          ...
                          {chapterQuizzes.map((quiz) => (
                            <Card key={quiz.id} className="p-3 border-dashed border-[var(--accent)]/30 hover:bg-[var(--accent-light)]/20 transition-all flex items-center justify-between group">
                              ...
                              <Link href={`/teacher/quizzes/${quiz.id}/builder`}>
                                <Button size="sm" variant="ghost" icon={<ExternalLink size={12} />}>Edit Quiz</Button>
                              </Link>
                            </Card>
                          ))}
