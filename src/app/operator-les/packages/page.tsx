...
export default function PackagesPage() {
  ...
  const handleEdit = (pkg: any) => {
    ...
    setLearningMode(pkg.learning_mode || "Online");
    setProgramType(pkg.program_type || "Reguler");
    setShowModal(true);
  };
  ...
  const resetForm = () => {
    ...
    setLearningMode("Online"); setProgramType("Reguler");
  };
          ...
          {packages.map(pkg => (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={pkg.id} className={`bg-white rounded-2xl border ${pkg.is_active ? 'border-orange-200' : 'border-slate-200'} shadow-sm overflow-hidden flex flex-col`}>
              <div className={`p-6 ${pkg.is_active ? 'bg-orange-50/50' : 'bg-slate-50'} border-b border-slate-100 flex-1`}>
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-2 leading-tight">{pkg.name}</h3>
                <div className="flex flex-wrap gap-2 mb-4">
                  <span className="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded-md">{pkg.level} {pkg.major ? `• ${pkg.major}` : ''}</span>
                  <span className={`text-xs font-bold px-2 py-1 rounded-md ${pkg.learning_mode === 'Online' ? 'bg-teal-50 text-teal-600' : 'bg-orange-50 text-orange-600'}`}>{pkg.learning_mode === 'Online' ? 'Online' : 'Tatap Muka'}</span>
                  <span className={`text-xs font-bold px-2 py-1 rounded-md ${pkg.program_type === 'Premium' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>{pkg.program_type || 'Reguler'}</span>
                </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">Mode Belajar</label>
                  <select value={learningMode} onChange={e=>setLearningMode(e.target.value)} className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-orange-500 outline-none">
                    <option>Online</option><option>Center</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">Tipe Program</label>
                  <select value={programType} onChange={e=>setProgramType(e.target.value)} className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-orange-500 outline-none">
                    <option>Reguler</option><option>Premium</option>
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
