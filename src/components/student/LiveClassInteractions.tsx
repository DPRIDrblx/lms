...
export default function StudentLiveInteractions({ scheduleId, studentId }: { scheduleId: string, studentId: string }) {
  ...
  useEffect(() => {
    ...
    const fetchInitial = async () => {
      // Notes
      const { data: noteData } = await supabase.from('class_shared_notes').select('*').eq('schedule_id', scheduleId).eq('student_id', studentId).single();
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      ...
      const { data: existing } = await supabase.from('class_shared_notes').select('id').eq('schedule_id', scheduleId).eq('student_id', studentId).single();
      
      if (existing) {
        await supabase.from('class_shared_notes').update({ notes }).eq('id', existing.id);
      } else {
        await supabase.from('class_shared_notes').insert({ schedule_id: scheduleId, student_id: studentId, notes });
      }
    ...
    <>
      ...
      <Card className="p-6 border-amber-100 bg-gradient-to-b from-white to-amber-50/30">
        ...
        <textarea
          ...
          placeholder="Buat catatan pribadimu di sini selama kelas berlangsung. Catatan ini akan tersimpan otomatis..."
