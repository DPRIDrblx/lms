...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "primary", size = "md", loading, icon, children, disabled, ...props }, ref) => {
    ...
    const variants: Record<string, string> = {
      primary:
        "bg-[var(--text-primary)] text-white hover:bg-black focus-visible:ring-black shadow-[var(--shadow-sm)] active:scale-[0.98]",
      secondary:
        "bg-white text-[var(--text-primary)] border border-[var(--border)] hover:bg-[var(--bg-secondary)] hover:border-[var(--border-hover)] hover:shadow-sm active:scale-[0.98]",
      ghost:
